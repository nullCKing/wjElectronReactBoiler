        ...
        { type: 'separator' },
        {
          ...
          click: async () => {
            let savePath = await dialog.showSaveDialog({
              defaultPath: path.join(__dirname, `brokerage-list-${moment().format('YYYY-MM-DD-HH-mm-ss')}.txt`),
            });
            ...
            if (!savePath.canceled) {
              ...
              if (filePath !== undefined) {
                fs.writeFile(filePath, "Your data here", function(err) {
                  if (err) {
    ...
    const templateDefault = [
      {
        ...
        submenu: [
          ...
          },
          {
            ...
            accelerator: 'Ctrl+X',
            click: async () => {
              this.mainWindow.webContents.send('show-intro');
              let savePath = await dialog.showSaveDialog({
                defaultPath: path.join(__dirname, `brokerage-list-${moment().format('YYYY-MM-DD-HH-mm-ss')}.txt`),
              });
